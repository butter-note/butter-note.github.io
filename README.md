...
아티클·무료 강의는 페이지당 6개, 템플릿은 9개씩 표시합니다. 이전·다음과 페이지 번호로 이동할 수 있고, 검색·필터를 변경하면 첫 페이지로 돌아갑니다. 검색은 현재 페이지뿐 아니라 전체 콘텐츠를 대상으로 합니다. 페이지가 하나뿐이거나 결과가 없으면 페이지 버튼을 숨깁니다. JavaScript가 꺼져 있으면 기존처럼 전체 목록을 표시합니다.
