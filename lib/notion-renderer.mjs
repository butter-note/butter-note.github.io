...
function rehypeNotionPresentation({ idPrefix = 'notion' } = {}) {
  return (tree) => {
    const headings = [];
    ...
    function visit(parent) {
      ...
      parent.children = parent.children.map((node) => {
        ...
        if (idPrefix !== 'notion' && originalId) node.properties.id = `${idPrefix}-${originalId}`;
        if (/^h[1-6]$/.test(node.tagName)) {
          node.properties.id = `${idPrefix}-heading-${headings.length + 1}`;
          headings.push({ id: node.properties.id, title: plainText(node), depth: Number(node.tagName[1]) });
        }
        if (originalId) idMap.set(String(originalId), node.properties.id);
        if (node.tagName === 'a' && String(node.properties.href ?? '').startsWith('#')) anchors.push(node);
    ...
    for (const anchor of anchors) {
      const target = idMap.get(String(anchor.properties.href).slice(1));
      if (target) anchor.properties.href = `#${target}`;
    }
 ...
 * @param {{title?: string, flavor?: 'notion' | 'markdown', idPrefix?: string}} options
 * @returns {import('react-markdown').Options}
 */
export function notionMarkdownOptions(content, { title, flavor = 'notion', idPrefix = 'notion' } = {}) {
  return {
    children: content,
    remarkPlugins: [remarkGfm, remarkMath, [remarkNotion, { flavor }], [remarkArticleTitle, { title }]],
    rehypePlugins: [rehypeRaw, [rehypeSanitize, schema], [rehypeKatex, { trust: false }], [rehypeNotionPresentation, { idPrefix }]],
  };
}
