...
/** Assets for a click-to-open player. Only mount playbackSrc after the user opens it.
 * @param {string} value
 */
export function getVideoPlayerAssets(value) {
  const embed = getVideoEmbed(value);
  let playbackSrc = embed.src;
  let poster;
  if (embed.kind === 'iframe' && embed.src) {
    const player = new URL(embed.src);
    player.searchParams.set('autoplay', '1');
    playbackSrc = player.href;
    if (embed.provider === 'YouTube') {
      const id = player.pathname.split('/').at(-1);
      poster = `https://i.ytimg.com/vi/${id}/hqdefault.jpg`;
    }
  }
  return { ...embed, playbackSrc, poster };
}
